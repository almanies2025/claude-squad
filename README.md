...
On rotation, the engine writes the target account's credentials to the Keychain and updates `~/.claude/.credentials.json` so Claude Code picks up the new credentials. The next API call seamlessly uses the new account.
